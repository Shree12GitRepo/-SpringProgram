...
public class FormValidation implements Validator{
	...
	public void validate(Object target, Errors errors) {
		...
		if(cmd.getPadd().length()==0 || cmd.getPadd()==null || cmd.getPadd()=="")
		{
			errors.rejectValue("padd","com.PatientDetails.Vallidator.padd" );
		}
		 if(cmd.getPadd().length()<0 || cmd.getPadd().length()>20)
		{
		 ...
		 * errors.rejectValue("regeistrationdate",
		 * "com.PatientDetails.Vallidator.regeistrationdate"); }
		 */
